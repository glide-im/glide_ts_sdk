...
export class WsDialog implements RtcDialog {
    ...
    constructor(peer: Peer, dialer: boolean, signaling: WsSignaling) {
        ...
        this.removeMessageListener = this.signaling.addMessageListener(
            (m: SignalingMessage) => {
...
export class WsDialing implements Dialing {
    ...
    private dialOnce() {
        ...
        this.signaling
            .sendSignaling(this.peerId, SignalingType.Dialing, this.myInfo)
            .then(() => {})
            .catch((e) => {
                this.cancel().then((r) => {});
...
export class WsIncoming implements Incoming {
    ...
    constructor(peer: PeerInfo, signaling: WsSignaling) {
        ...
        this.removeMessageListener = signaling.addMessageListener(
            (m: SignalingMessage) => {
    ...
    async accept(): Promise<RtcDialog> {
        ...
        await this.signaling.sendSignaling(
            this.peerInfo.id,
    ...
    reject(): Promise<void> {
        ...
        return this.signaling
            .sendSignaling(this.peerInfo.id, SignalingType.Reject, {
                id: this.signaling.myId,
            })
            .finally(() => {
                this.disposeEvent.next('cancel');
