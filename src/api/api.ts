...
import {
    ...
    MidBean,
    ServerInfoBean,
    SessionBean, TicketBean,
    UserInfoBean
} from "./model";
import { rxios } from "./rxios";

function login(account: string, password: string, v2: boolean): Observable<AuthBean> {
    const param = {
        Email: account,
        Device: 2,
        Password: password
    };
    return rxios.post("auth/signin" + (v2 ? "_v2" : ""), param);
}
...
function getMessageHistry(
    uid: string,
    beforeMid: number
): Observable<MessageBean[]> {
    return rxios.post("msg/chat/history", {
        Uid: parseInt(uid),
        Before: beforeMid
    });
}
...
function getTicket(to: string): Observable<TicketBean> {
    return rxios.post("session/ticket", { To: to });
}
