...
class IMWebSocketClient extends WsClient {
    ...
    public request<T>(
        ...
        return this.sendProtocolMessage(message).pipe(
            ...
            map((m: CommonMessage<any>) => {
                if (
                    m.action === Actions.NotifyError ||
                    m.action === Actions.ApiFailed
                ) {
    ...
    public sendCliCustomMessage(
        m: CliCustomMessage
    ): Observable<CliCustomMessage> {
    ...
    public sendRecallMessage(m: Message) {
        return of(
            this.createCommonMessage(m.to, Actions.MessageChatRecall, m)
        ).pipe(
            mergeMap((msg) => this.sendProtocolMessage(msg)),
    ...
    private sendProtocolMessage<T>(
        data: CommonMessage<T>
    ): Observable<CommonMessage<T>> {
    ...
    private getSendFailedObservable(
        msg: CommonMessage<any>
        ...
        return this.messages().pipe(
            ...
            filter(
                (m) =>
                    m.action === Actions.NotifyError ||
                    m.action === Actions.NotifyForbidden
            ),
            take(1),
            map((m) => {
    ...
    // 接收者回执消息, 用于确认消息发送成功
    private getAckNotifyObservable(
        msg: Message
    ): Observable<MessageSendResult> {
        return this.messages().pipe(
    ...
    private getApiRespObservable<T>(seq: number): Observable<CommonMessage<T>> {
        return this.messages().pipe(
            ...
            filter(
                (m) =>
                    m.action === Actions.ApiSuccess ||
                    m.action === Actions.ApiFailed ||
                    m.action === Actions.NotifySuccess ||
                    m.action === Actions.NotifyError
            ),
            take(1)
        );
