...
export function ChatMessageItem(props: { msg: ChatMessage }) {
    ...
    useEffect(() => {
        if (
            msg.Type === MessageType.UserOffline ||
            msg.Type === MessageType.UserOnline
        ) {
            return;
        }
...
function MessageStatusView(props: { message: ChatMessage }) {
    ...
    if (
        ...
        props.message.Sending === SendingStatus.ClientAck
    ) {
        return <></>;
    }
    ...
    return (
        <Box
            ...
            height={'100%'}
            px={1}>
            <Tooltip title={hint}>{statusEle}</Tooltip>
        </Box>
    );
}
            ...
            return (
                <Box
                    ...
                    <Box
                        ...
                        alignItems={'center'}
                        className={
                            'rounded-md bg-gray-100 py-2 cursor-pointer hover:bg-amber-50'
                        }>
                        <Typography
                            variant={'caption'}
                            mx={1}
                            component={'span'}
                            color={grey[500]}>
                            {props.msg.getReplyMessage()?.getSenderName()}:{' '}
                            {props.msg
                                .getReplyMessage()
