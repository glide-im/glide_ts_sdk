            ...
            <AppBar
                ...
                <Toolbar>
                    <Box sx={{ flexGrow: 1 }}>
                        ...
                        {typing ? (
                            <Typography variant='caption' color={'grey'} ml={2}>
                                输入中...
                            </Typography>
                        ) : (
                            <></>
...
export function ChatRoomContainer() {
    ...
    if (session === null) {
        return (
            <Box className={'rounded-br-md rounded-tr-md'}>
    ...
    return (
        <Box
            className={
                'h-full max-h-full flex flex-col rounded-br-md rounded-tr-md'
            }
            style={{
            ...
            <Box
                className={'flex-none grow-0 w-full rounded-tr-md'}
                color={'black'}
                bgcolor={'white'}>
                <SessionTitleBar session={session} />
            </Box>
            <Box
                className={'w-full flex-1 flex-shrink'}
                height={`calc(100% - 64px - ${inputHeight}px)`}>
                <SessionMessageList />
            </Box>
...
export const ChatRoomContainerMobile: any = withRouter(
    (props: RouteComponentProps) => {
        ...
        const sendMessage = (msg: string, type: number) => {
            if (session != null) {
                session
                    .send(msg, type)
                    .subscribe({ error: (err) => showSnack(err.toString()) });
            }
        };
                ...
                <Box
                    ...
                    <div className={'flex flex-col h-full'}>
                        <Box
                            ...
                            className={
                                'BeautyScrollBar overflow-y-auto flex w-full'
                            }>
                            <SessionMessageList />
                        </Box>
                    </div>
