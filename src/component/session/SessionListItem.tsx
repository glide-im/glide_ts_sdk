...
export function SessionListItem(props: {
    chat: Session;
    onSelect: (c: Session) => void;
}) {
    const { sid } = useParams<{ sid: string }>();

    const [sessionInfo, setSessionInfo] = useState<SessionBaseInfo>({
        ...props.chat,
    });
        ...
        const sp = props.chat.messageSubject.subscribe((msg) => {
            if (!msg.FromMe && props.chat.ID !== sid) {
                if (props.chat.Type === SessionType.Channel) {
                    showSnack(
                        `[${props.chat.Title}] ${msg.getSenderName()}: ${
        ...
        <>
            <ListItemButton
                ...
                <ListItemIcon>
                    <Badge
                        variant={'standard'}
                        badgeContent={sessionInfo.UnreadCount}
                        overlap='rectangular'
                        color={'secondary'}>
                        <Avatar
                            variant={'circular'}
