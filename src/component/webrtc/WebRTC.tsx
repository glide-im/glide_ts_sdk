...
export function WebRtcView(props: { targetId: string; onClose: () => void }) {
    ...
    const [incoming, setIncoming] = React.useState<Incoming | null>(
        WebRTC.incoming
    );
    const [dialing, setDialing] = React.useState<Dialing | null>(null);
    const [call, setCall] = React.useState<RtcDialog | null>(null);

    const [rtcState, setRtcState] = React.useState<WebRtcSessionState>(
        WebRTC.incoming === null ? 'idle' : 'incoming'
    );
        ...
        switch (rtcState) {
            case 'idle':
                WebRTC.dial(props.targetId)
                    .then((dialing) => {
                        dialing.peer.onLocalMediaReady = (
                            stream: MediaStream
                        ) => {
                            videoRef.current!.srcObject = stream;
                        };
                        dialing.peer.onRemoteTrack = (track: RTCTrackEvent) => {
                            videoTargetRef.current!.srcObject =
                                track.streams[0];
                        };
                        setDialing(dialing);
                        setRtcState('dialing');
